...
-- 2) Accounts (upsert theo email cho tiện seed lại)
INSERT INTO auth_accounts (email, phone, status, email_verified_at, created_at, updated_at) VALUES
  ('admin@example.com',       '+84901111111', 'active', now() - interval '30 days', now() - interval '30 days', now() - interval '1 day'),
  ('candidate0@example.com',  '+84902222222', 'active', now() - interval '5 days',  now() - interval '5 days',  now() - interval '2 hours'),
  ('candidate1@example.com',  '+84901111111', 'active', now() - interval '4 days',  now() - interval '4 days',  now() - interval '2 hours'),
  ('candidate2@example.com',  '+84902222222', 'active', now() - interval '4 days',  now() - interval '4 days',  now() - interval '2 hours'),
  ('candidate3@example.com',  '+84903333333', 'active', now() - interval '4 days',  now() - interval '4 days',  now() - interval '2 hours'),
  ('recruiter0@example.com',  '+84913333333', 'active', now() - interval '5 days',  now() - interval '1 day',   now() - interval '1 day')
...
-- 3) Password credentials (NOT NULL => không để rỗng)
INSERT INTO auth_password_credentials (account_id, password_hash, password_algo, password_updated_at)
SELECT a.account_id, v.password_hash, v.password_algo, v.password_updated_at
FROM auth_accounts a
JOIN (VALUES
  ('admin@example.com',      '$2b$12$C6UzMDM.H6dfI/f/IKcXeOZ8b0k2q0m8wJcQp5GmYkCq6l9yq9D2G', 'bcrypt', now() - interval '1 day'),
  ('candidate0@example.com', '$2b$12$k7qvEJ9qv2c9h6i2G2pX1e7h6sHk7yJ0jv1l2m3n4o5p6q7r8s9t0u', 'bcrypt', now() - interval '2 hours'),
  ('candidate1@example.com', '$2b$12$k7qvEJ9qv2c9h6i2G2pX1e7h6sHk7yJ0jv1l2m3n4o5p6q7r8s9t0u', 'bcrypt', now() - interval '2 hours'),
  ('candidate2@example.com', '$2b$12$k7qvEJ9qv2c9h6i2G2pX1e7h6sHk7yJ0jv1l2m3n4o5p6q7r8s9t0u', 'bcrypt', now() - interval '2 hours'),
  ('candidate3@example.com', '$2b$12$k7qvEJ9qv2c9h6i2G2pX1e7h6sHk7yJ0jv1l2m3n4o5p6q7r8s9t0u', 'bcrypt', now() - interval '2 hours'),
  ('recruiter0@example.com', '$2b$12$yW8u7t6s5r4q3p2o1n0m9l8k7j6h5g4f3e2d1c0b9a8Z7Y6X5W4V', 'bcrypt', now() - interval '1 day')
) AS v(email, password_hash, password_algo, password_updated_at)
  ON v.email = a.email
...
-- 5) Sessions (UUID unique; thêm 1 revoked session)
...
-- 6) Refresh tokens
-- token_prefix là VARCHAR(24) và UNIQUE => dùng prefix đúng 24 ký tự để tránh lỗi.
INSERT INTO auth_refresh_tokens
  (token_id, session_id, account_id, token_prefix, token_hash, expires_at, created_at, rotated_at, revoked_at)
SELECT
  v.token_id, v.session_id, a.account_id, v.token_prefix, v.token_hash,
...
FROM (VALUES
  ...
  -- token cho session đã revoked (revoked_at set)
